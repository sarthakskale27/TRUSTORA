...
export const Sidebar = ({ activeTab, setActiveTab, isOpen, onClose }) => {
  const navItems = [
    ...
    { id: 'trust-radar', label: 'Trustora Intelligence', icon: ShieldCheck, badge: '96%' },
    { id: 'onboarding', label: 'Add Property Wizard', icon: PlusCircle, badge: 'Wizard' },
    ...
    { id: 'pricing', label: 'Dynamic Pricing Engine', icon: CalendarCheck, badge: 'Active' },
    ...
    <>
      ...
      <aside className={`fixed lg:static top-0 bottom-0 left-0 w-64 bg-slate-950 border-r border-slate-800/80 z-50 flex flex-col transition-transform duration-300 ${
        ...
        <div className="p-4 border-t border-slate-800/80">
          <div className="p-3.5 rounded-xl bg-gradient-to-br from-slate-900 to-slate-950 border border-emerald-500/20 shadow-md">
            <div className="flex items-center gap-2 mb-1">
              <ShieldCheck className="w-4 h-4 text-emerald-400" />
              <span className="text-xs font-bold text-white">Trustora Intelligence</span>
            </div>
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Biometric host badges, review burst & fraud radar running 24/7.
            </p>
          </div>
        </div>
      </aside>
