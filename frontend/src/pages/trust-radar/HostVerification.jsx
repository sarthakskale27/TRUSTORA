...
import {
  ...
  Loader2, FileText, UserCheck, Lock, Award, RefreshCw, ArrowRight, Zap, Check
} from 'lucide-react';
...
export const HostVerification = () => {
  ...
  useEffect(() => {
    api.get('/trust/host/verification/status')
      .then(res => {
        if (res.data) {
          setIsVerified(res.data.verified || res.data.status === 'verified');
          setVerStatus(prev => ({
            ...prev,
  ...
  const handleRunVerification = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setStep(5);
        showToast('Govt ID & Biometric Face-Match Verified Successfully! (Verified Host ✓ Issued)', 'success');
      }, 1200);
    } catch {
      setTimeout(() => {
        setIsVerified(true);
        setVerifyingAnimation(false);
        setStep(5);
        showToast('Verification signals processed (DEMO MODE)', 'success');
      }, 1000);
    } finally {
  ...
  const steps = [
    ...
    { num: 4, title: 'Biometric AI Checks' },
    { num: 5, title: 'Verified Badge ✓' },
  ];
    ...
    <div className="space-y-6 max-w-5xl mx-auto animate-fadeIn pb-12">
      ...
      <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-emerald-950/40 border border-emerald-500/30">
            <div className="w-10 h-10 rounded-xl bg-emerald-600 flex items-center justify-center text-white font-black text-sm shadow">
              ✓
            </div>
            <div>
              <p className="text-xs font-bold text-white">{isVerified ? 'Verified Host ✓' : 'Pending Verification'}</p>
              <p className="text-[10px] text-emerald-400 font-semibold">Confidence: {verStatus.confidence}%</p>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="px-4 py-3 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:opacity-90 text-white font-bold text-xs shadow flex items-center gap-1.5 cursor-pointer transition-all shrink-0"
          >
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />}
            <span>1-Click Complete KYC</span>
          </button>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center justify-between overflow-x-auto gap-2">
        {steps.map((s) => {
          const isDone = s.num < step || isVerified;
      ...
      <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl space-y-6">
        {step === 1 && (
          <div className="space-y-4 max-w-xl animate-fadeIn">
            ...
            <button
              onClick={() => setStep(2)}
              className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs cursor-pointer transition-all flex items-center gap-1.5"
            >
              <span>Next: Upload Document</span> <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
        )}
          ...
          <div className="space-y-4 max-w-xl animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-slate-950 border border-dashed border-emerald-500/40 text-center space-y-2">
                ...
                <p className="text-[10px] text-slate-400">aadhaar_front_scan.jpg</p>
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-dashed border-emerald-500/40 text-center space-y-2">
                ...
                <p className="text-[10px] text-slate-400">aadhaar_back_scan.jpg</p>
                <span className="inline-block px-2 py-0.5 rounded bg-emerald-950 text-emerald-400 text-[10px] font-bold">Uploaded ✓</span>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300 text-xs font-semibold">Back</button>
              <button onClick={() => setStep(3)} className="px-6 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5">
                <span>Next: Selfie Snapshot</span> <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 max-w-xl animate-fadeIn">
            ...
            <div className="p-6 rounded-2xl bg-slate-950 border border-slate-800 flex items-center gap-4">
              ...
              <div className="space-y-1">
                <p className="text-xs font-bold text-white">Liveness Check: Ready</p>
                <p className="text-[11px] text-emerald-400 font-semibold">3D Depth & Glare Verification: Normal</p>
                <p className="text-[10px] text-slate-500">Document photo & selfie aligned</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setStep(2)} className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300 text-xs font-semibold">Back</button>
              <button
                onClick={handleRunVerification}
                disabled={loading}
                className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow flex items-center gap-2 cursor-pointer"
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                <span>Run Biometric & Document Verification →</span>
              </button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-6 max-w-xl animate-fadeIn py-4 text-center">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-black text-white">Analyzing Biometric & Document Signals...</h3>
              <p className="text-xs text-slate-400">Verifying OCR microprint, holographic markers, and 128-point face landmarks.</p>
            </div>
            <div className="space-y-2 text-left max-w-sm mx-auto">
              <div className="flex items-center justify-between text-xs font-semibold text-emerald-400">
                <span>UIDAI Document Check</span>
                <span>99.1% Matched</span>
              </div>
              <div className="flex items-center justify-between text-xs font-semibold text-teal-400">
                <span>Face-Match Alignment</span>
          ...
          <div className="space-y-6 animate-fadeIn">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-slate-950 border border-emerald-500/30 space-y-1">
                ...
                <p className="text-2xl font-black text-emerald-400">{verStatus.idAuthenticity}%</p>
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-emerald-500/30 space-y-1">
                ...
                <p className="text-2xl font-black text-teal-400">{verStatus.faceMatchScore}%</p>
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-emerald-500/30 space-y-1">
                ...
                <p className="text-2xl font-black text-emerald-400">{verStatus.confidence}%</p>
                <p className="text-[10px] text-emerald-300">✓ Trustora Verified Badge Active</p>
              </div>
            </div>

            {/* Results Callout */}
            <div className="p-6 rounded-2xl bg-gradient-to-r from-emerald-950/60 via-slate-950 to-teal-950/60 border border-emerald-500/40 flex flex-col md:flex-row items-center justify-between gap-4">
